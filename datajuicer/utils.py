...
def _run(data, func, args, kwargs, run_mode="normal", cache_mode="normal", cache_dir="Sessions/", store_key=None):
    data = copy.copy(data)
    if type(func) == str:
        func = get(data,func)
...
def run(grid, func,n_threads=1, run_mode="normal", cache_mode="normal", cache_dir="Sessions/", store_key=None):
    def _runner(grid, *args, **kwargs):
        ...
        gens = [_run(data,func,list(args),kwargs,run_mode, cache_mode, cache_dir, store_key) for data in grid]

        for gen in gens:
            print(next(gen))

        if n_threads==1:
            return [next(gen) for gen in gens]

        with concurrent.futures.ThreadPoolExecutor(max_workers=n_threads) as executor:
            futures = [executor.submit(next, gen) for gen in gens]
