...
thread_proxies = {"__stdout__":{}, "__stderr__":{}, "stdout":{}, "stderr":{}}
...
def redirect(file):
    ...
    thread_proxies["__stdout__"][ident] = Multiplexer(orig___stdout__, file)
    thread_proxies["__stderr__"][ident] = Multiplexer(orig___stderr__, file)
    thread_proxies["stdout"][ident] = Multiplexer(orig_stdout, file)
    thread_proxies["stderr"][ident] = Multiplexer(orig_stderr, file)
...
def stop_redirect():
    ...
    # close/remove the buffer
    thread_proxies["__stdout__"][ident].close()
    del thread_proxies["__stdout__"][ident]
    thread_proxies["__stderr__"][ident].close()
    del thread_proxies["__stderr__"][ident]
    thread_proxies["stdout"][ident].close()
    del thread_proxies["stdout"][ident]
    thread_proxies["stderr"][ident].close()
    del thread_proxies["stderr"][ident]


def _get_stream(original, name):
    """
    ...
    def proxy():
        ...
        return thread_proxies[name].get(ident, original)
...
def enable_proxy():
    ...
    objects.
    """
    sys.__stdout__ = LocalProxy(_get_stream(sys.__stdout__, "__stdout__"))
    sys.__stderr__ = LocalProxy(_get_stream(sys.__stderr__, "__stderr__"))
    sys.stdout = LocalProxy(_get_stream(sys.stdout, "stdout"))
    sys.stderr = LocalProxy(_get_stream(sys.stderr, "stderr"))
